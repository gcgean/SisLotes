...
describe("encargos no pagamento de despesas",()=>{
  ...
  it("persiste componentes e limpa no estorno",()=>{const s=readFileSync(resolve(__dirname,"../../backend/src/routes/modules/despesas.ts"),"utf8");expect(s).toContain("multa_paga = multa.toFixed");expect(s).toContain('desconto_obtido = "0.00"');});
});
