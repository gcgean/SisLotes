...
describe("extrato do fechamento",()=>{
  ...
  it("exibe o fechamento no formato do extrato de lançamentos",()=>{const component=read("src/components/financeiro/FechamentoPeriodoTab.tsx");for(const text of["Saldo inicial do período","Entradas no período","Saídas no período","Saldo final do período","O período acumulou caixa","O período queimou caixa","Contas a receber não recebidas","Contas a pagar não pagas","Total ainda não consolidado","Imprimir extrato","Todas as contas","Buscar descrição, conta ou loteamento"])expect(component).toContain(text)});
});
