...
async function migrate() {
    ...
    const mysqlConn = await promise_1.default.createConnection({
    ...
    try {
        ...
        const [usuarios] = await mysqlConn.execute('SELECT * FROM usuarios');
        for (const u of usuarios) {
            const userMaster = (u.user_master === 's' || u.user_master === 'S' || u.user_master === '1');
            const mapPerm = (val) => (val === 's' || val === 'S' || val === '1');
            try {
        ...
        const [contas] = await mysqlConn.execute('SELECT * FROM contas');
      ...
      `, [c.id_conta, c.apelido, c.titular, c.agencia, c.conta, c.convenio]);
        ...
        const [clientes] = await mysqlConn.execute('SELECT * FROM clientes');
        for (const c of clientes) {
            ...
            if (tipo === 'j') {
                ...
                if (c.pj_cep)
                    cep = c.pj_cep;
                if (c.pj_complemento)
                    complemento = c.pj_complemento;
                if (c.pj_fone_com)
                    fone_com = c.pj_fone_com;
            }
            const emptyToNull = (val) => (val && typeof val === 'string' && val.trim() !== '') ? val.trim() : null;
            const params = [
                c.id_cliente,
                tipo,
                c.nome,
        ...
        const [loteamentos] = await mysqlConn.execute('SELECT * FROM loteamentos');
      ...
      `, [
        ...
        const [lotes] = await mysqlConn.execute('SELECT * FROM lotes');
        for (const l of lotes) {
            const toVarchar20 = (val) => {
        ...
        const [vendas] = await mysqlConn.execute('SELECT * FROM vendas');
        ...
        const [pagamentos] = await mysqlConn.execute('SELECT * FROM pagamentos');
        let count = 0;
        ...
        `, [
            ...
            if (count % 5000 === 0)
                console.log(`Migrados ${count} pagamentos...`);
        }
