...
async function gerarRelatorio(client) {
...
function validarMapeamento(valor) {
    if (!valor || typeof valor !== "object" || Array.isArray(valor))
        throw new Error("Mapeamento deve ser um objeto JSON.");
    const entrada = valor;
    for (const [grupo, chave] of [["recebimentos", "id_pagamento"], ["despesas", "id_despesa_parcela"]]) {
        const itens = entrada[grupo] ?? [];
        if (!Array.isArray(itens))
            throw new Error(`${grupo} deve ser uma lista.`);
        ...
        for (const item of itens) {
            ...
            if (vistos.has(id))
                throw new Error(`${grupo}: ${chave} ${id} está duplicado.`);
            vistos.add(id);
        }
    }
    return entrada;
}
async function aplicarMapeamento(client, mapa, confirmar) {
    await client.query("BEGIN");
    try {
        const alteracoes = [];
        for (const item of mapa.recebimentos ?? []) {
            const result = await client.query(`SELECT p.id_empresa, c.id_conta
         FROM pagamentos p JOIN contas c ON c.id_conta = $2 AND c.id_empresa = p.id_empresa AND c.ativo = true
         WHERE p.id_pagamento = $1 AND p.situacao = 'pago' AND p.id_conta IS NULL FOR UPDATE OF p`, [item.id_pagamento, item.id_conta]);
            if (result.rowCount !== 1)
                throw new Error(`Recebimento ${item.id_pagamento} não está pago/sem conta ou a conta é inválida para a empresa.`);
        ...
        for (const item of mapa.despesas ?? []) {
            const result = await client.query(`SELECT dp.id_empresa, c.id_conta
         FROM despesa_parcelas dp JOIN contas c ON c.id_conta = $2 AND c.id_empresa = dp.id_empresa AND c.ativo = true
         WHERE dp.id_despesa_parcela = $1 AND dp.situacao = 'pago' AND dp.id_conta IS NULL FOR UPDATE OF dp`, [item.id_despesa_parcela, item.id_conta]);
            if (result.rowCount !== 1)
                throw new Error(`Despesa paga ${item.id_despesa_parcela} não está paga/sem conta ou a conta é inválida para a empresa.`);
...
async function main() {
    ...
    try {
        if (applyIndex >= 0) {
            ...
            if (!arquivo)
                throw new Error("Informe o arquivo após --apply.");
            const mapa = validarMapeamento(JSON.parse(await (0, promises_1.readFile)((0, node_path_1.resolve)(arquivo), "utf8")));
            console.log(JSON.stringify(await aplicarMapeamento(client, mapa, confirmar), null, 2));
            if (!confirmar)
                console.log("Simulação concluída com rollback. Use --confirm somente após revisar o resultado.");
        }
        else {
