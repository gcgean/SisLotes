...
exports.assistenteRouter = void 0;
const express_1 = require("express");
const zod_1 = require("zod");
const auth_1 = require("../../middleware/auth");
const AuditoriaService_1 = require("../../services/AuditoriaService");
const assistente_1 = require("../../services/ia/assistente");
const deepseek_1 = require("../../services/ia/deepseek");
const provider_1 = require("../../services/ia/provider");
exports.assistenteRouter = (0, express_1.Router)();
exports.assistenteRouter.use(auth_1.requireAuth);
/**
 ...
 */
let provedorCache = null;
function obterProvedor() {
    if (!provedorCache)
        provedorCache = new deepseek_1.ProvedorDeepSeek();
    return provedorCache;
}
const corpoSchema = zod_1.z.object({
    pergunta: zod_1.z.string().min(1, "Escreva uma pergunta").max(2000),
    // Histórico da conversa, enviado pelo frontend e limitado para conter custo.
    historico: zod_1.z
        .array(zod_1.z.object({
        papel: zod_1.z.enum(["usuario", "assistente"]),
        conteudo: zod_1.z.string().max(4000),
    }))
        .max(20)
        .optional(),
});
...
function dentroDoLimite(idEmpresa) {
    ...
    if (atual.chamadas >= LIMITE_POR_HORA)
        return false;
    atual.chamadas += 1;
    return true;
}
// ─── GET /status — o frontend usa para exibir (ou esconder) o assistente ─────
exports.assistenteRouter.get("/status", (_req, res) => {
...
exports.assistenteRouter.post("/", async (req, res) => {
    ...
    const historico = (parse.data.historico ?? []).map((m) => m.papel === "usuario"
        ? { papel: "usuario", conteudo: m.conteudo }
        : { papel: "assistente", conteudo: m.conteudo });
    try {
        const resultado = await (0, assistente_1.responder)(obterProvedor(), parse.data.pergunta, { usuario, idEmpresa }, historico);
        console.log(`[assistente] empresa=${idEmpresa} usuario=${usuario.login} ` +
        ...
        for (const acao of resultado.acoesExecutadas) {
            ...
            await AuditoriaService_1.AuditoriaService.registrar(req, tabela, "CREATE", acao.id, undefined, acao.resumo, `Criado pelo assistente de IA a pedido de ${usuario.login}: "${parse.data.pergunta.slice(0, 120)}"`);
        }
        return res.json({
        ...
        if (e instanceof provider_1.ErroProvedorIA) {
