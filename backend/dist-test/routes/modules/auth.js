...
async function resolveHubOnLoginIfNeeded(args) {
    ...
    empresa.hub_cache_until = new Date(Date.now() + (Boolean(resolved.canAccess) ? 60000 : 10000));
    empresa.hub_features = HubBillingService_1.HubBillingService.withHubMeta((resolved.features && typeof resolved.features === "object" && !Array.isArray(resolved.features)
        ? resolved.features
        : empresa.hub_features) ?? {}, {
        ...
        return res.json({
            ...
            licenca: empresa
                ? {
                    ...
                    plano: empresa.plano,
                    daysLeft: HubBillingService_1.HubBillingService.getStoredDaysLeft(empresa),
                }
                : null,
        });
    }
...
exports.authRouter.post("/esqueci-senha", async (req, res) => {
    try {
...
exports.authRouter.post("/refresh", auth_1.requireAuth, (req, res) => {
    const user = req.user;
    if (!user)
