...
import { verificarPeriodoFinanceiro, verificarPeriodoFinanceiroContas, verificarPermissaoRetroativa } from "../../services/PeriodoFinanceiroService";
...
lancamentosRouter.post("/transferencias", async (req: AuthRequest, res: Response) => {
  ...
  const bloqueioNovo=await verificarPeriodoFinanceiroContas(idEmpresa,parse.data.data,[parse.data.id_conta_origem,parse.data.id_conta_destino]);if(bloqueioNovo)return res.status(409).json({error:bloqueioNovo});
...
lancamentosRouter.put("/transferencias/:id", async (req: AuthRequest, res: Response) => {
  ...
  if (!parse.success) return res.status(400).json({ error: "Dados inválidos", issues: parse.error.issues });
  const idEmpresa = req.user!.id_empresa;
  const bloqueio=await verificarPeriodoFinanceiroContas(idEmpresa,parse.data.data,[parse.data.id_conta_origem,parse.data.id_conta_destino]);if(bloqueio)return res.status(409).json({error:bloqueio});
  const retroativo=await verificarPermissaoRetroativa(req.user!,parse.data.data);if(retroativo)return res.status(403).json({error:retroativo});
  if (!(await validarContasTransferencia(idEmpresa, [parse.data.id_conta_origem, parse.data.id_conta_destino]))) {
    return res.status(400).json({ error: "As contas de origem e destino devem pertencer à empresa." });
  }
  ...
  const bloqueioAntigo=await verificarPeriodoFinanceiroContas(req.user!.id_empresa,transferencia.data,[transferencia.id_conta_origem,transferencia.id_conta_destino]);if(bloqueioAntigo)return res.status(409).json({error:bloqueioAntigo});
...
lancamentosRouter.delete("/transferencias/:id", requirePermission("financeiro_excluir"), async (req: AuthRequest, res: Response) => {
  ...
  if (!transferencia) return res.status(404).json({ error: "Transferência não encontrada" });
  const bloqueio=await verificarPeriodoFinanceiroContas(req.user!.id_empresa,transferencia.data,[transferencia.id_conta_origem,transferencia.id_conta_destino]);if(bloqueio)return res.status(409).json({error:bloqueio});
  const valoresAntigos = { id_conta_origem: transferencia.id_conta_origem, id_conta_destino: transferencia.id_conta_destino, descricao: transferencia.descricao, valor: transferencia.valor, data: transferencia.data };
...
lancamentosRouter.post("/", async (req: AuthRequest, res: Response) => {
  ...
  const idEmpresa = req.user!.id_empresa;
  const bloqueio=await verificarPeriodoFinanceiro(idEmpresa,data.data,data.id_conta);if(bloqueio)return res.status(409).json({error:bloqueio});
...
lancamentosRouter.put("/:id", async (req: AuthRequest, res: Response) => {
  ...
  const bloqueio=await verificarPeriodoFinanceiroContas(req.user!.id_empresa,parse.data.data??lancamento.data,[lancamento.id_conta,parse.data.id_conta??lancamento.id_conta]);if(bloqueio)return res.status(409).json({error:bloqueio});
...
lancamentosRouter.delete("/:id", requirePermission("financeiro_excluir"), async (req: AuthRequest, res: Response) => {
  ...
  if (!lancamento) return res.status(404).json({ error: "Lançamento não encontrado" });
  const bloqueio=await verificarPeriodoFinanceiro(req.user!.id_empresa,lancamento.data,lancamento.id_conta);if(bloqueio)return res.status(409).json({error:bloqueio});
