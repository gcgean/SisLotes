...
function assertEqual(nome: string, atual: number, esperado: number) {
  if (Math.abs(atual - esperado) > 0.001) throw new Error(`${nome}: esperado ${esperado}, obtido ${atual}`);
}
...
async function main() {
  ...
  try {
    ...
    const conta = await client.query(
      `INSERT INTO contas (id_empresa, apelido, tipo, saldo_inicial, data_saldo_inicial, ativo)
       VALUES ($1, 'TESTE', 'caixa', 1000, CURRENT_DATE, true) RETURNING id_conta`,
      [id_empresa],
    );
    const idConta = conta.rows[0].id_conta;
    const despesa = await client.query(
      `INSERT INTO despesas (id_empresa, id_categoria, descricao, valor_total, numero_parcelas, recorrente, recorrencia_ativa)
       VALUES ($1, $2, 'Cenário Fase 1', 300, 3, false, false) RETURNING id_despesa`,
      [id_empresa, id_categoria],
    );
    const idDespesa = despesa.rows[0].id_despesa;
    const parcelas = await client.query(
      `INSERT INTO despesa_parcelas (id_empresa, id_despesa, numero_parcela, vencimento, valor, situacao)
       ...
       RETURNING id_despesa_parcela, numero_parcela, TO_CHAR(vencimento, 'DD/MM/YYYY') vencimento`,
      [id_empresa, idDespesa],
    );
    const parcela1 = parcelas.rows.find((p) => p.numero_parcela === 1);
    const parcela2 = parcelas.rows.find((p) => p.numero_parcela === 2);
    await client.query(
      `UPDATE despesa_parcelas SET situacao='pago', pago_data=CURRENT_DATE, valor_pago=100, id_conta=$2, id_usuario=$3
       WHERE id_despesa_parcela=$1`,
      [parcela1.id_despesa_parcela, idConta, id_usuario],
    );
    ...
    const recebimento = await client.query(
      `INSERT INTO pagamentos (id_empresa,id_venda,id_conta,id_usuario,numero_parcela,tipo,situacao,vencimento,valor,pago_data,valor_pago,multa,juros,reajustado)
       VALUES ($1,$2,$3,$4,$5,'carne','pago',CURRENT_DATE,200,CURRENT_DATE,200,0,0,false) RETURNING id_pagamento`,
      [id_empresa, id_venda, idConta, id_usuario, proxima.rows[0].numero],
    );
    await client.query(
      `INSERT INTO lancamentos_manuais (id_empresa,id_conta,tipo,id_conta_contabil,descricao,valor,data,id_usuario)
       VALUES ($1,$2,'despesa',$3,'Débito manual cenário',50,CURRENT_DATE,$4)`,
      [id_empresa, idConta, id_categoria, id_usuario],
    );
    const totais = await client.query(
      `SELECT COALESCE(SUM(valor) FILTER (WHERE tipo='receita'),0) receita,
       ...
       FROM movimentos_financeiros WHERE id_conta=$1`,
      [idConta],
    );
    ...
    const atrasadas = await client.query(
      `SELECT COUNT(*)::int qtd FROM despesa_parcelas WHERE id_despesa=$1 AND situacao='aberto' AND vencimento<CURRENT_DATE`,
      [idDespesa],
    );
    assertEqual("card atrasadas", Number(atrasadas.rows[0].qtd), 1);

    await client.query(
      `UPDATE despesa_parcelas SET situacao='aberto', pago_data=NULL, valor_pago=NULL, id_conta=NULL WHERE id_despesa_parcela=$1`,
      [parcela1.id_despesa_parcela],
    );
    const saldoAposEstorno = await client.query(
      `SELECT 1000 + COALESCE(SUM(CASE WHEN tipo='receita' THEN valor ELSE -valor END),0) saldo
       FROM movimentos_financeiros WHERE id_conta=$1`,
      [idConta],
    );
