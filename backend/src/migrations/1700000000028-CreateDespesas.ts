...
export class CreateDespesas1700000000028 implements MigrationInterface {
  name = "CreateDespesas1700000000028";
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const valuesSql = CATEGORIAS_DESPESA_PADRAO.map(
      ({ nome, grupo }) => `('${nome.replace(/'/g, "''")}', '${grupo}')`
    ).join(",\n        ");
    await queryRunner.query(`
